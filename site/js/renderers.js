(function () {
  ...
  function headerMarkupResponsive(currentPage, route) {
    const profile = store.siteData.profile;
    const nav = currentUI().nav;
    const copy = currentUI();
    const inputValue = route.page === "search" ? route.query : state.searchQuery;
    ...
    const closeLabel = state.lang === "en" ? "Close" : "关闭";
    const menuLabel = state.lang === "en" ? "Open navigation menu" : "打开导航菜单";
    const menuCloseLabel = state.lang === "en" ? "Close navigation menu" : "关闭导航菜单";
    const searchLabel = state.lang === "en" ? "Open search" : "打开搜索";
    const searchCloseLabel = state.lang === "en" ? "Close search" : "关闭搜索";
    const searchSubmitSymbol = "⌕";
    const navigationMarkup = items
      .map(
        ([key, label]) => `
          <button class="nav-link ${currentPage === key ? "active" : ""}" data-route="${key}">
            ${escapeHtml(label)}
      ...
      .join("");
    const mobileNavigationMarkup = items
      .map(
        ([key, label]) => `
          <button class="mobile-drawer-link ${currentPage === key ? "active" : ""}" data-route="${key}">
            ${escapeHtml(label)}
          </button>`
      )
      .join("");
    const languageSwitchMarkup = (className = "") => `
      <div class="lang-switch ${className}">
        <button class="lang-btn ${state.lang === "zh" ? "active" : ""}" data-lang="zh">中</button>
        <button class="lang-btn ${state.lang === "en" ? "active" : ""}" data-lang="en">EN</button>
      </div>
    `;
    const searchShellMarkup = (inputId, suggestionsId, className = "") => {
      const suggestionMarkup = searchSuggestions(inputValue)
        .map(
          (item) => `
            <button class="search-suggestion" type="button" role="option" aria-selected="false" data-search-suggestion="${escapeHtml(item.label)}" ${item.route ? `data-search-route="${escapeHtml(item.route)}"` : ""}>
      ...
      return `
        <div class="search-shell ${className}" data-search-shell role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-owns="${suggestionsId}">
          <label class="sr-only" for="${inputId}">${escapeHtml(copy.search)}</label>
          <form class="search-box" data-search-form>
            <input id="${inputId}" class="search-input" type="search" name="q" value="${escapeHtml(inputValue)}" placeholder="${escapeHtml(copy.searchPlaceholder)}" aria-label="${escapeHtml(copy.search)}" aria-controls="${suggestionsId}" aria-autocomplete="list" aria-expanded="false" autocomplete="off">
            <button class="search-submit" type="submit" aria-label="${escapeHtml(copy.search)}">${searchSubmitSymbol}</button>
          </form>
          <div id="${suggestionsId}" class="search-suggestions" data-search-suggestions role="listbox" aria-label="${escapeHtml(searchSuggestionsLabel())}">
            ${suggestionMarkup}
          </div>
        </div>
      `;
    ...
    return `
      <header class="nav">
        <div class="nav-mobile-bar">
          <button class="nav-toggle" type="button" data-mobile-menu-toggle aria-expanded="${state.mobileMenuOpen ? "true" : "false"}" aria-controls="mobile-nav-drawer" aria-label="${escapeHtml(state.mobileMenuOpen ? menuCloseLabel : menuLabel)}">
          ...
          <div class="nav-mobile-actions">
            <button class="mobile-search-trigger" type="button" data-mobile-search-toggle aria-expanded="${state.mobileSearchOpen ? "true" : "false"}" aria-controls="mobile-search-layer" aria-label="${escapeHtml(state.mobileSearchOpen ? searchCloseLabel : searchLabel)}">${searchSubmitSymbol}</button>
            ${languageSwitchMarkup("lang-switch-compact")}
          </div>
        </div>
        <nav class="menu">
          ${navigationMarkup}
        ...
        <div class="nav-tools">
          ${searchShellMarkup("global-search-input", SEARCH_SUGGESTIONS_ID)}
          ${languageSwitchMarkup()}
        </div>
        ${state.mobileMenuOpen || state.mobileSearchOpen ? `<button class="mobile-overlay-backdrop" type="button" data-mobile-close aria-label="${escapeHtml(closeLabel)}"></button>` : ""}
        ${state.mobileMenuOpen ? `<div class="mobile-drawer" id="mobile-nav-drawer">${mobileNavigationMarkup}</div>` : ""}
        ${state.mobileSearchOpen
          ? `
            <div class="mobile-search-layer" id="mobile-search-layer">
              <div class="mobile-search-head">
                <div class="meta">${escapeHtml(copy.search)}</div>
                <button class="mobile-search-close" type="button" data-mobile-close aria-label="${escapeHtml(searchCloseLabel)}">×</button>
              </div>
              ${searchShellMarkup("mobile-search-input", "mobile-search-suggestions", "search-shell-mobile")}
            </div>
          `
          : ""}
      </header>
  ...
  function worksMarkup() {
    ...
    return `
      <section class="section works-shell">
        ...
        <div class="grid-3">
          ${filtered.map((item) => workCardMarkup(item)).join("")}
        </div>
      </section>
    `;
  }
  ...
  function homeMarkupResponsive() {
    const siteData = store.siteData;
    const profile = siteData.profile;
    const copy = currentUI().home;
    const featuredWorks = siteData.works.slice(0, 6);
    const recentExhibitions = sortByYearDesc(siteData.exhibitions).slice(0, 1);
    const heroTitle = state.lang === "en" ? profile.hero.title_en : profile.hero.title_zh;
    const heroSubtitle = state.lang === "en" ? profile.hero.subtitle_en : profile.hero.subtitle_zh;
  ...
  Object.assign(site, {
    searchEntries,
    searchSuggestions,
    headerMarkup: headerMarkupResponsive,
    contactMarkup,
    footerMarkup,
    workCardMarkup,
    ...
    newsCardMarkup,
    yearRailMarkup,
    homeMarkup: homeMarkupResponsive,
    dropdownMarkup,
    worksMarkup,
